...
| 21 | **Owner refinements to the chapters — do not reverse these.** Dark/night, route, environment, film frames, signature, dates, transitions, content status, Bhagira, social links, media, performance, and the "one continuous expedition" rule. | **Dark and night sections** stay dark (gateway, Singapore, South Korea) but are never a plain black screen: very dark *charcoal* (`#0e141d`→`#0a0f18` gateway; `#0b1116`/`#0c0d15` night heroes), a faint terrain texture, a pool of atmospheric light, a blue route glow (a wide faint underlay stroke — never a blur filter), restrained red for the completed route, and (superseded by #22) a static film grain. Singapore is modern, precise, controlled: architectural grid, range rings, a skyline with its own waterline **reflection**, distant city lights. South Korea is cinematic night driving: a perspective highway, lane marks, city lights, red/white light trails kept to the right of the type; a daytime photograph is taken down to dusk rather than overlaid. **Page 3 (`JourneyMap`) is not touched.** **Route:** BLUE = road ahead / active journey, RED = travelled / completed memory, and the change is *progressive with scroll*: the gateway's blue road turns red country by country as the visitor scrolls (scrubbed, reversible; the km counter follows), the arrival road draws red under the traveller, the leaving map fades this country blue→red. The next blue road is already on screen when a chapter opens (glowing dotted line leaving the hero from the first frame; a blue stub on the interlude and transition). **Environment page (was "challenge"):** hazards are *geography, never events*. Beat renamed `environment`; the tag reads "Regional terrain" (Indonesia: volcanic geography, ambient embers) or "Natural environment" (Taiwan: seismic terrain, seismograph) — never anything like danger encountered; a test forbids eruption/felt/struck/escaped wording. No earthquake or eruption is implied to have been experienced unless a manuscript later confirms it. **Film frames (Page 10):** a frame with no photograph is a deliberate stand-in — the country outline in warm print tones, the location name, the route coordinates and "Memory 01" (the frame number; the owner's "memory slot", worded so it never reads as a task label) — and the moment `gallery` has a photograph for that frame the frame swaps to it by itself; the page also appears for a country whose only memories are gallery photographs. **Signature (Page 11):** illustrations stay, labelled "Visual interpretation"; a real photograph (`signatureImage`) takes the same stage with the same entrance and the label "Photograph" (+ credit). **Dates:** `arrivalDate`/`departureDate` are optional on every country and shown exactly as supplied; the arrival row is LOCATION · COORDINATES · DATE, and with no date it is LOCATION · COORDINATES — no empty slot, no separator (`arrivalRows()`, tested). No date is invented; none exists yet. **Indicative routes:** every route is captioned only by a small "Indicative route" tag inside the map, not a paragraph; only Singapore→Indonesia is identified as a sea crossing (dashed, "SEA CROSSING") — no other crossing is called ferry/air/land without evidence. **Counters and timelines** are correct from any arrival (reload half way, back/forward, anchor jump, resize/orientation, a fling, opening directly on a route): ScrollTrigger applies an already-scrolled position with tween callbacks *suppressed*, so counter text is also written from `onRefresh`/`onUpdate` (`withSync`), and 16 e2e scenarios on Chromium and mobile-safari pin it. **Country differentiation:** on top of #20's colour, terrain and effect, every country now has an *environment scene* (`scenes` in `atmospheres.ts`, drawn by `lib/silhouette.ts`): Cambodia stepped temple towers and warm dust; Singapore skyline and reflected lights; Indonesia volcano cones; Taiwan a mountain range over a dense city; China three vast layered massifs; South Korea a highway; Australia a lone road to an empty horizon; Sri Lanka coast; and Europe country by country (Switzerland ragged alpine peaks, Austria peaks and spires, Germany pines, Italy hills and cypress, Slovenia lakes, Croatia coast, Hungary plains, France rolling hills, Slovakia Carpathian ridges) — never one Europe theme. Motion differs too: long flat scenes and light trails drift *sideways* (`data-axis="x"`), mountains and buildings rise. **Transitions (a major priority):** the hand-off (`TransitionLink`) plays as one move in the next country's colours — the red road exits the frame → the next blue road appears → the coordinates tick from here to there → the next country's terrain morphs in → its chapter line, name and coordinates enter *exactly where the next hero's will be* — and the hero, seeing the veil, does not replay its type, so the name simply stays and the veil lifts. A sourced sea crossing turns the road into a dashed navigation line (in the veil, the hero and the transition page); nothing is claimed as a confirmed land crossing, so land transitions keep the road visually continuous. **The journey rail** (`ChapterRail`, fixed to the bottom of every chapter) always shows where we came from, where we are and where we go next — names, chapter number, coordinates, 23 ticks (red ridden, blue ahead, current ringed) and this chapter's scroll progress; it steps aside at the very end of a page. **Content status:** every story has `contentStatus: "pending" | "draft" | "verified"`; only `verified` copy is ever public (drafts render only in a preview build with `NEXT_PUBLIC_SHOW_DRAFTS=1`); a country with no story is `pending`. A pending chapter shows a *wordless atmospheric interlude* (its landscape, the red road running through it, chapter number and coordinates) instead of any developer placeholder — no copy, so nothing to invent and nothing to remove later. **Bhagira** is a dedicated unresolved item (`content/bhagira.ts`, `ChapterBhagira.tsx`): who/what, relationship, story, dates, location and importance are **not guessed anywhere**; the entry is `null`, the slot renders nothing, and one verified object naming a country adds a film-frame section to that chapter with no restructuring. **Social links:** `socialConfig = { instagramUrl: "", youtubeUrl: "" }`; a profile is listed only with a real https URL, an empty one hides the button everywhere, never a guessed profile. **Media object:** `coverImage`, `heroImage?`, `gallery[]`, `signatureImage?`, `videoUrl?` per country, read only through `lib/media.ts` (which ignores any path with no file on disk) — components never hard-code an image path, and replacing Cambodia/Singapore/China's terrain covers is one data change. **Performance:** one ScrollTrigger and one timeline per particle group instead of one per particle (a chapter holds 14–21 triggers, not ~60); particles CSS-hidden on a phone are neither painted nor animated (half of every layer is dropped); no blur filters (glows are wide faint strokes; the grain is a static image); media lazy-loads via `next/image`; the journey rail updates one CSS variable from a passive rAF-throttled scroll listener; **no Three.js / WebGL**; a test navigates between chapters and asserts ScrollTriggers do not accumulate (`window.__rowMotion.triggers()`). **Experience rule:** the site is one continuous expedition, not 23 web pages — the visitor always knows where we came from, where we are, where we go next, through route continuity, chapter numbers, coordinates and the rail. **Caught in testing:** hover labels on the gateway; the odometer/ferry clock stuck at 0 when arriving already scrolled (#20); a `publicStory` self-recursion from a blind find-and-replace; peaks that read as triangles and temples that read as pines (both redrawn after looking at them); the seismograph running through Taiwan's paragraph. |
| 22 | **Clean-up: the site looked dirty, so quiet beats rich.** Overrides the film grain and ambient effects asked for in #21. | The owner's verdict on the running site was that every page looked dirty. Looking at it as a visitor confirmed it, and the causes were mostly noise added in #20/#21, plus some older defects. **Removed:** the film grain overlay (`Grain`), and the ambient effects on every hero except the two night countries (dust, mist bands, streaks and petals read as smudges and scratches; countries now have `effect: "none"`, Singapore keeps a few city lights — 18 — and South Korea a few light trails — 6). **Toned down:** hero contour lines (opacity ≤ 0.14, was up to 0.34), the environment silhouettes (65% weight, 40% behind a photograph), the gateway terrain texture (0.14). **Fixed older defects:** the homepage map and the story-card maps had an SVG that did not fill its frame, so page cream showed as a stripe / a rectangle inside a rectangle (the frames now share the ocean colour; the map treatment itself is unchanged); a thick red swoosh drawn over the Support photo (removed); a straight glowing red line across the homepage hero photo (now a thin 2.5px line, no glow); the finale headline sitting on a busy photo (a left scrim behind it, and a red that reads on navy); and the Stories page setting a six-line summary as its headline (each story now has its short headline, with the summary as body text). **Rule going forward:** every decorative layer must earn its place; if a screen can lose a layer and still say the same thing, it should. Nothing in the chapters' structure, data or behaviour changed. |
| 23 | **Full redesign to "Clean & light": the owner said the whole site looked dirty even after #22's clean-up, and asked to change the whole site.** Supersedes #16–#22 wherever they conflict — the "Red Line Across a Blue World" cinematic direction, the dark gateway/night heroes, the country-by-country environment scenes, the scroll choreography, the hand-off veil and the journey rail are all removed, not toned down. | Asked directly rather than guessed again: **look** = "Clean & light" (white pages, big photographs, plain black headlines, one blue accent, generous space); **motion** = "Almost none" (a simple fade as content scrolls into view; no pinned scrolling, no map drawing itself, no transition veil). Both are now the standing design direction until the owner says otherwise. **Removed outright:** GSAP and every hand-built scroll-choreography file (`choreography.ts`, `useChapterMotion.ts`, `Scene`'s old motion engine, `TransitionLink`'s hand-off veil, `ChapterRail`, `ChapterInterlude`, `TerrainBackdrop`, `EffectLayer`, `SceneLayer`, `Grain`, `lib/contours.ts`, `lib/silhouette.ts`, `lib/gsap.ts`, `components/motion/*`) — the package no longer depends on `gsap`/`@gsap/react` for anything shipped. The dark gateway and the Singapore/South Korea night treatment are gone; every page is white or soft grey, plain black type (`--ink: #111418`), one blue accent (`--support`) for every button, link and most of the map, and red (`--route`) reserved for "ridden" — on the world map, and nowhere else as decoration. **The one remaining motion is CSS-only**: a shared `.fade` class using `animation-timeline: view()` (`@supports`-gated, so it simply does nothing — content is just always visible — in any browser without support, which was Safari/Firefox at the time this was written), itself disabled under `prefers-reduced-motion: reduce`. No JavaScript scroll listener runs anywhere on the site any more. **Per-chapter identity is now a single accent colour, not a page-level colour, terrain or effect**: `atmospheres.ts` collapsed from `{theme, hero, terrain, scenes, effect, environment}` to `{theme, environmentLabel}`; the ten signature-moment illustrations (Falls, Ferry, Skyline, etc. — kept, per the owner's earlier "keep the illustrations" instruction) are now static SVG with their `--story-*` colours scoped only to their own small box, not the page, so a chapter never has a colour "mood" beyond that one illustration. **Every chapter page was rewritten** to the same plain pattern as the homepage: a photo-or-terrain-map hero, a flat grey/red/blue regional route map for arrival and departure, plain text blocks for the road/environment/discovery/people/signature pages, and a plain "Enter Sri Lanka" / "Follow the journey" link at the bottom with a Previous/All-chapters row — no veil, no rail, no pinned scroll. The gateway map (`/journey`) is now a plain white section with the whole ridden route already drawn in red and a dashed blue stub after Slovakia — nothing to wait for or scroll to complete. `BookFeature`'s pointer-tilt effect is gone along with it. **A real, unrelated bug was found and fixed while rebuilding `JourneyMap`** (Page 3, otherwise untouched per #21): its info panel `<div>` was nested *inside* the map's own clipped, positioned frame — a leftover from an earlier design where the panel was a `position: absolute` glass overlay on top of the map. Restyling the panel to sit in normal flow below the map (rather than floating over it) didn't move it in the JSX, so it kept rendering inside the map's box, visually on top of the SVG and physically intercepting clicks meant for the map — confirmed with `document.elementFromPoint`, not guessed. Fixed by moving the panel to be a sibling of the map frame. **Caught in testing:** a full-page (`fullPage: true`) Playwright screenshot renders the CSS scroll-linked fade animation as if scroll never happened (the capture doesn't simulate scrolling), making later sections look permanently blank in a screenshot even though they render correctly for a real scrolling visitor — confirmed by querying `getComputedStyle().opacity` directly instead of trusting the screenshot; this is a screenshot-capture artifact, not a site bug, and is worth remembering the next time a `fullPage` capture looks wrong. Several e2e tests (`reduced-motion.spec.ts`, parts of `chapters.spec.ts`) tested motion/veil/rail behaviour that no longer exists and were rewritten or removed rather than left to fail. |
| 24 | **World map (Page 3): every visited country red at once, not gated behind stepping through Next/Previous. Story cards: the map-outline illustration removed.** | Direct owner request. Found, while fixing it, that the map's colouring had drifted from its own original brief (decision #18: "every visited country filled solid red") to being gated behind `reachedCount = activeIndex + 1` — so on load only India showed red, and a visitor had to click "Next country" 22 times to see the whole route. Fixed: every journey country is unconditionally `reached` (red); `activeIndex` now only controls which country's panel and marker are focused, never which countries are coloured. Added a smooth once-only fade-in (the same `.fade` CSS pattern as the rest of the site) on the reached-countries group as the map scrolls into view, and a `transition` on the active country's outline stroke. Story cards (`StoryCard`, used on the homepage and `/stories`) lost their small country-outline map image entirely — cards are text-only now (label, headline, link); the real photo or terrain-map fallback still lives on the chapter page itself. |
| 25 | **Real photographs sourced from the manuscript PDF for Cambodia and China; Singapore deliberately left with its terrain-map fallback.** | Owner asked to "pick the pics from the [manuscript]" for the top-10 story pages. `pymupdf` (already available in the environment) rendered and text-searched *The Indian Cyclist - July 04.pdf* to confirm each country's page range from `journey.ts`'s existing `source.pages` citations lined up with the manuscript's actual page labels (they do, almost exactly), then extracted embedded images at native resolution from those pages — not screenshots. **Used:** a group photo of Ranjith and a fellow rider with local children outside a temple (Cambodia, p. 97 — matches the verified Buddhism/temples summary) and a photo of Ranjith with two fellow cyclists on a tree-lined road (China, p. 116 — matches the verified local-hospitality summary); both cropped to the site's 4:3 hero ratio and set as `coverImage`, with real descriptive `coverAlt` text replacing the old "standing in" placeholder, and an `// OWNER:` note on each asking for the other riders/location to be confirmed and for confirmation that the manuscript's image licensing covers the website (a book's photo licence does not automatically cover republishing on a public site). **Not used, and deliberately left as the terrain-map fallback:** Singapore's manuscript pages (105–110) contain only generic skyline/landmark photography (Marina Bay Sands, Sentosa, Chinatown) with no photo of Ranjith in any of them — likely licensed stock rather than his own photography, so none of it was used. Also explicitly not used anywhere: a Phnom Penh skyline photo on the Cambodia pages visibly watermarked "© Kim Seng \| CaptainKimo.com" — a third party's credited stock photo. **Also found, not acted on:** `TRAVEL PORTFOLIO.pdf` (a separate 8-page bio/media-kit document in the repo root) lists social handles and follower counts (YouTube "RanjithOnWheels" 379,000+, Instagram @ranjithonwheels 754,000+, a ranjithonwheels.com URL) — but its own headline numbers (19 countries, 47,000+ km, 5 years, and a country list that includes Turkey but omits half the verified 23) contradict the already-verified `site.ts` facts, so it reads as an older or generic bio rather than an authoritative source. Not wired into `socials.ts` — flagged to the owner instead of guessed in, per CLAUDE.md §1. |
| 26 | **24th country (Czech Republic) and official YouTube/Instagram/X links confirmed by the owner and added.** | Following on from #25's flag: asked the owner to "take internet data" to check the handles found in `TRAVEL PORTFOLIO.pdf` before using them, given this project's own history of nearly rebuilding around a same-named but different real person earlier in the session (see the top-of-session identity guardrail) — a near-identical risk pattern (a different real "Ranjith on Wheels," different stats) surfaced again and had to be checked, not assumed. Web search resolved it: the "Solution to Pollution" message and "The Indian Cyclist" book title are exact, specific matches only this Ranjith would have, and news coverage traces one continuous person from a 2021 Warangal cycle-yatra (tribute to his late father) through to a current European leg — genuinely the same subject this site is about, not a repeat mix-up. A vlog ("VLOG-74, COUNTRY #24 🇨🇿") independently confirmed a 24th country, Czech Republic, dated 2026-09-21 — flagged to the owner rather than added unilaterally (CLAUDE.md's own rule: facts here are owner-confirmed, not scraped). Owner confirmed both on 2026-09-23. **Country #24:** added to `journey.ts` (order 24, `slug: "czech-republic"`, ISO 203 added to `countryShape.ts` so its map outline renders), `atmospheres.ts` (a fresh theme, contrast-checked), `site.ts` (`countryCount: 24`, `latestCountry: "Czech Republic"`). No manuscript content, no city, no date — `latestCity` is now `string | undefined` (was a required field) and every consumer (`ChapterDeparture.tsx`) handles its absence instead of rendering `undefined`; `distanceKm` (48,000) was deliberately *not* touched, since no newer verified figure exists — it may now be an undercount. Slovakia's chapter automatically stopped being "the road isn't finished" and started leading on to Czech Republic; Czech Republic's chapter automatically became the new unfinished ending — both are derived from array order, so no component needed editing for this. **Social links:** `socials.ts`'s `SocialLink.id` gained a third channel (`"x"`); `socialConfig` now holds the three confirmed URLs (`youtube.com/@Ranjithonwheels`, `instagram.com/ranjithonwheels`, `x.com/ranjith_on`) instead of empty strings — every consumer (footer, mobile menu, finale, `/contact`) already mapped generically over `socialLinks`, so no component changed. **Deliberately not added:** a website URL — two domains surfaced (`ranjithonwheels.com` and `.in`) alongside clearly spam-pattern mirror URLs (`ranjithonwheels.com.smsnot.com`, a `ww7.` subdomain with tracking params), and there was no way to tell which of the two real-looking domains is canonical; and any follower-count numbers, since CLAUDE.md already requires those to be manually edited, not scraped, and a hardcoded scrape goes stale immediately. |
| 27 | **"Clean & light" (#23) partially reversed: a cinematic, photography-first documentary redesign, on direct owner instruction.** Supersedes #23's "almost no motion" and per-chapter-identity rules wherever they conflict; #23's white/black/one-blue-accent palette, plain typography and no-dark-sections rules all stand. | The owner asked for the homepage to read as "a premium travel documentary... interactive digital travel book... photography-first," explicitly rejecting the card-grid pattern #23 had settled on, and separately asked for a real UPI payment experience (amount selection, a dynamically generated QR, a deep link) in place of the static-only panel. Both are large, deliberate reversals of recent decisions, logged the same way every previous reversal in this file has been. **Earlier in the same session, before this instruction arrived:** the chapter hand-off was changed so the last country's "Next" link goes to `/support` before the Follow finale (`ChapterTransition.tsx`, `SupportFinalCTA.tsx`) rather than straight to Follow; the homepage's featured-3-cards-plus-button section was widened to list all 24 chapters directly (`StoryCard` reused on both `/` and `/stories`); `StoryCard` itself was redesigned twice more — first into a full photographic card (the whole card a link, country-accent-tinted label, reusing `atmospheres.ts`), then given a visible caption (the photo's own `coverAlt` text, not new copy) and a hover reveal. All of that is superseded by this decision's own homepage rebuild below, except `StoryCard` itself, which is unchanged and still serves `/stories`. **Hero (`HeroJourney.tsx`):** rebuilt full-viewport, the same real photograph (`open-road.jpg`) as before, a cursor-depth parallax on the image (desktop, real pointer only, off under reduced motion — `HeroParallax.tsx`), a slow Ken Burns scale (CSS `@keyframes`, disabled globally under reduced motion), a one-time SVG route-line draw-in, a count-up on the three site.ts stats (`CountUp.tsx`) that always **renders the true final value first** and only plays a from-zero animation once in view and motion is allowed — never a "0" a script might fail to replace — and a scroll cue into the chapters below. **Caught and fixed during layout, not after:** the header is `position: sticky` and reserves its own 72px of flow space, so `min-height: 100svh` on the hero always overflowed the first screen by exactly the header's height (confirmed by measuring `getBoundingClientRect()`, not by eyeballing) — fixed to `calc(100svh - var(--header-height))`; a large top padding meant for a top-aligned layout was still being counted into the height of a *bottom*-aligned flex item, silently eating the room meant for the stats row — removed; the scroll cue's "SCROLL" label collided with the stats row at the same bottom-left corner — moved to bottom-right, clear of it. **Chapters (`ChapterJourney.tsx` / `ChapterSection.tsx` / `ChapterProgressRail.tsx`):** the card grid is gone; the same ten documented countries `/stories` already uses (a full summary and a page-verified citation) now each get one large, alternating photo/text section — the country's own moments from `stories.ts` become the body paragraphs, its `signature`-beat moment (or its first moment) becomes a pull quote, its `atmospheres.ts` accent tints the chapter number and label, and a Previous/Progress/Next nav closes each one — nothing here is written fresh, all of it already existed in `journey.ts`/`stories.ts`. A sticky desktop rail (dots + "NN / 10") tracks which chapter is on screen via one shared `IntersectionObserver` per concern (one for "which chapter," one for "is any chapter visible at all") rather than one observer per section, and fades out entirely outside the chapters section — it does not float over the Origin or Map screens above. The fourteen countries without a full story are **not** forced into this format; they stay reachable from `/journey` exactly as before. **Reveal-on-scroll (`Reveal.tsx`, `useInView.ts`):** a small `IntersectionObserver` hook, not GSAP and not the site's existing scroll-timeline `.fade` (kept, unchanged, still used elsewhere) — chosen because this redesign needed the reveal to work in every browser, not only the Chromium engines `animation-timeline: view()` supports. Its hidden-before-reveal state only ever applies under a `.js` class that `layout.tsx` sets from an inline script the moment it actually runs (a new, one-line addition to the root layout) — a visitor with JS off, or a script that fails, has no `.js` ancestor and simply sees the finished section, the same lesson this file's own decision log already learned once for reduced motion, applied here before it could repeat. **UPI payment panel (`SupportUPI.tsx`):** amount buttons (₹100 / ₹500 / ₹1,000 / Custom, with a validated whole-rupee custom field — empty, negative, zero and non-numeric all rejected before reaching the link), a `upi://pay?...` deep link built by a new pure helper (`buildUpiUri` in `content/support.ts`) from the site's **existing, owner-confirmed** UPI ID — never a new or invented one — and a QR that switches: no amount selected still shows the original supplied JPEG completely unmodified (CLAUDE.md §9.2's plain-`<img>`, no-filter, no-transform, no-border-radius rules untouched, verified against the actual rendered CSS, not assumed), while picking an amount swaps to a freshly generated code (`qrcode.react`, newly installed — no QR library existed in the project before; chosen for being a small, pure-SVG, dependency-light component, not canvas or WASM) encoding that exact amount, so the two never conflate "the original photograph" with "a generated graphic." **The brief's `src/config/support.ts` with an empty `upiId` placeholder was deliberately not created**: a real UPI ID already exists and is already live at `content/support.ts`'s `upiConfig` (`qrUpiId`/`manualUpiId`, both owner-confirmed per decision #16) — adding a second, empty config next to a working one would be the confusing, misleading outcome the brief's own "never invent a destination" instruction was written to prevent, not a step toward it. **Verified after implementing, not assumed:** full lint/typecheck/unit-test/production-build/e2e passes (203 e2e, 1 skip, matching the pre-existing baseline exactly — nothing newly skipped or newly failing); the full page was screenshotted and actually read at 1440px and 390px, at the top of the hero, mid-chapter, and on the UPI panel with an amount selected and a custom amount entered, specifically because a `fullPage` Playwright screenshot is known (this file's own decision log) to render mid-reveal content as blank — these were viewport-sized, scrolled captures, not `fullPage`, for exactly that reason. Console errors across every route were checked directly: the only ones found are the same pre-existing 400s for countries with no cover photograph yet (Sri Lanka, Thailand, Singapore, the Europe leg…), already handled by `FallbackImage`'s terrain-map swap, not a regression. **Left exactly as this file already required:** no dark sections, no new page-level colour moods, `--ink`/`--support`/`--route` unchanged, `.fade`/CSS-only scroll-timeline untouched for everything outside the new chapters. |

---

## 1. Project truth

Single editable source at `src/content/site.ts`. Never hard-code any of this in a component. Superseded by decision #26 (owner-confirmed 24 countries, Czech Republic the latest) — the snippet below is historical; read `site.ts` itself.
...
export const site = {
  ...
  distanceKm: 48000,          // "48,000+" — not re-verified since #26; may be an undercount now
  countryCount: 24,
  latestCountry: "Czech Republic",
  latestCity: undefined,      // OWNER: no specific city confirmed yet
  years: 4,                   // "4+"
...
Superseded by decision #13 ("Sunset Adventure" palette), and superseded again by decision #23 ("Clean & light" — white surfaces, black type, one blue accent, red reserved for "ridden"). The snippet below is historical; `src/app/globals.css` is canonical — read it, don't trust this block.
...
- [x] Cambodia cover / hero — sourced from the manuscript PDF, decision #25. Needs the owner's confirmation (names, location, licence) noted in the `// OWNER:` comment beside it.
- [ ] Singapore cover / hero — the manuscript's Singapore pages have no photo of Ranjith, only generic stock landmark photography; none of it was used (decision #25). Still needed.
- [x] China cover / hero — sourced from the manuscript PDF, decision #25. Needs the owner's confirmation (names, location, licence) noted in the `// OWNER:` comment beside it.
- (`npm run content:status` also lists every other country that has no photograph yet — Sri Lanka, Thailand, Australia and the Europe leg among them. Those are not being requested until their manuscripts exist.)
...
**CONTENT REQUIRED** — each is a story entry in `src/content/stories.ts` (`contentStatus: "verified"` only once checked against the manuscript); until then the chapter has no road/environment/discovery/people/signature pages — only the always-present hero, arrival, leaving and transition.
- [ ] Sri Lanka manuscript
- [ ] Thailand manuscript
- [ ] Australia manuscript
- [ ] South Korea manuscript
- [ ] Remaining Europe manuscripts (France, Switzerland, Germany, Austria, Italy, Slovenia, Croatia, Hungary, Slovakia, Czech Republic)
...
- [x] Official YouTube URL (`youtubeUrl`) — confirmed by the owner, decision #26.
- [x] Official Instagram URL (`instagramUrl`) — confirmed by the owner, decision #26.
- [x] Official X URL (`xUrl`) — confirmed by the owner, decision #26.
- [ ] Official website URL — two candidate domains found (`.com`, `.in`) alongside spam-pattern mirrors; not added, canonical one still unconfirmed (decision #26).

**OPTIONAL, WHEN VERIFIED** — never estimated: crossing dates (`arrivalDate` / `departureDate`), which city in the Czech Republic (`site.latestCity`), whether any crossing besides Singapore→Indonesia was by sea, air or land (`seaCrossings` in `chapters.ts`), and whether `distanceKm` (48,000) still holds now that a 24th country has been ridden.
